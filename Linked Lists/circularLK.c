//circular singly linked list using multiple functions
#include <stdio.h>
#include <stdlib.h>
...
void traverseList(list *begin) {
	...
	printf(" %d ",begin->info);
	for(ptr=begin->next;ptr!=begin;ptr=ptr->next) {
		printf("-> %d ",ptr->info);
		count++;
	}
	printf("\n\n(%d) Nodes traversed.\n",count);		
...
list* searchInList(list *begin,int item) {
	...
	while((ptr->next!=begin)&&(ptr->info!=item)) {
		ptr=ptr->next;
		count++;
	}
	if(ptr->info==item) {
...
void insertAtBeg(list **begin,int item) {
	...
	if(*begin==NULL) {
		*begin=newNode;
		...
		list *ptr=*begin;
		for(;ptr->next!=*begin;ptr=ptr->next);
		ptr->next=newNode;
		newNode->next=*begin;
		*begin=newNode;
	}
...
void insertAtEnd(list **begin,int item) {
	...
	if(*begin==NULL) {
		*begin=newNode;
		...
		for(ptr=(*begin);ptr->next!=(*begin);ptr=ptr->next);
		ptr->next=newNode;
	}
	newNode->next=*begin;
...
void deleteNode(list **begin, int item) {
	...
	if(ptr==(*begin) && ptr->next==(*begin)) {
		*begin=NULL;
	}
	else {
		for(prev=*begin;prev->next!=ptr;prev=prev->next);
		prev->next=ptr->next;
		if(ptr==(*begin)) {
			*begin=ptr->next;
		}
